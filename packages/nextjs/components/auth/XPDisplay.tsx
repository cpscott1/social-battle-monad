...
export const XPDisplay = () => {
  ...
  const { nftStats, level } = useXP();

  if (!ready || !authenticated || !nftStats) return null;

  // Calculate progress percentage for the current level
  const progressPercentage = (nftStats.xp || 0) % 100;
  ...
  return (
    <div className="flex items-center gap-2 bg-base-300 px-3 py-1 rounded-full">
      ...
      <div className="flex flex-col">
        ...
        <span className="text-sm font-bold">{nftStats.xp || 0}</span>
      </div>
